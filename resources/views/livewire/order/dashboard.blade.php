    ...
    <div class="flex-1 grid gap-4">
        <div class="flex flex-wrap gap-3">
            <div class="p-6 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["all"]["count"]}} | {{$this->summary["all"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["all"]["name"]}}</p>
            </div>
            <div class="p-6 bg-gradient-to-r from-red-500 to-orange-600 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["hasDebt"]["count"]}} | {{$this->summary["hasDebt"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["hasDebt"]["name"]}}</p>
            </div>
            <div class="p-6 bg-gradient-to-r from-gray-500 to-gray-700 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["cancelled"]["count"]}} | {{$this->summary["cancelled"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["cancelled"]["name"]}}</p>
            </div>
            <div class="p-6 bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["inProgress"]["count"]}} | {{$this->summary["inProgress"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["inProgress"]["name"]}}</p>
            </div>
            <div class="p-6 bg-gradient-to-r from-green-400 to-green-600 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["completed"]["count"]}} | {{$this->summary["completed"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["completed"]["name"]}}</p>
            </div>
            <div class="p-6 bg-gradient-to-r from-teal-400 to-teal-600 rounded-lg shadow-md">
                <p class="text-3xl font-extrabold text-white">{{$this->summary["delivered"]["count"]}} | {{$this->summary["delivered"]["funds"]}}</p>
                <p class="text-lg font-medium text-gray-200 mt-1.5">{{$this->summary["delivered"]["name"]}}</p>
            </div>
        </div>
        ...
        <div class="grid gap-1">
            ...
            <div class="grid gap-2">
                @foreach(\App\Models\OrderStatus::orderBy("name","asc")->get() as $status)
                    <label for="order-status-{{$status->id}}" class="input">
