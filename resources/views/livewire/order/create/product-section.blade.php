    ...
    @if($orderItems[$index]["productId"] != "")
        <div class="flex items-center gap-1.5">
            ...
            <div class="relative grid flex-1">
                <input type="text" class="my-input" wire:model="orderItems.{{$index}}.receipt"
                       wire:click.outside="$set(receipts.{{$index}},[])"
                       wire:keyup="searchReceipts($event.target.value , {{$index}})">
                <div
                    class="my-input absolute w-full top-full mt-2 bg-white flex flex-wrap gap-2 {{count($receipts[$index]) > 0 ? "" : "hidden"}}">
