<div x-data="{state : $wire.entangle('state')}" x-show="state"
    ...
    @if(!is_null($payment))
        <div class="grid gap-1">
            ...
            <input type="text" class="input w-full" disabled value="{{$payment->pid ?? ""}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Qeyd</div>
            <textarea name="" class="input w-full" rows="3" disabled>{{$payment->note}}</textarea>
        </div>
        <div class="grid gap-1">
            <div class="my-label">Tarix</div>
            <input type="text" class="input w-full" disabled
                   value="{{$payment->created_at->format("d-m-Y h:i:s") ?? ""}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">İcraçı</div>
            <input type="text" class="input w-full" disabled value="{{$payment->executor->name ?? ""}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Müştəri</div>
            <input type="text" class="input w-full" disabled value="{{$payment->customer->name ?? ""}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Məbləğ</div>
            <input type="text" class="input w-full" disabled value="{{$payment->amount . " AZN" ?? ""}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Təsnifat</div>
            <input type="text" class="input w-full" disabled value="{{$payment->type->name}}">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Əməliyyat</div>
            <input type="text" class="input w-full" disabled value="{{$payment->action->name}}">
        </div>
        ...
        @if($payment->is_cancelled)
            <div class="grid gap-1">
                ...
                <input type="text" class="input w-full" disabled value="{{$payment->cancelledUser->name}}">
            </div>
            <div class="grid gap-1">
                <div class="my-label">Ləğv tarixi</div>
                <input type="text" class="input w-full" disabled
                       value="{{$payment->updated_at->format("d-m-Y h:i:s")}}">
            </div>
            <div class="grid gap-1">
                <div class="my-label">Səbəb</div>
                <textarea name="" class="input w-full" rows="3" disabled>{{$payment->explanation}}</textarea>
            </div>
        @endif
