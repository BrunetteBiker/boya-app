    ...
    <!--[if BLOCK]><![endif]--><?php if($orderItems[$index]["productId"] != ""): ?>
        ...
        <div class="flex justify-end gap-2">
            <button wire:click="addOrderItem"
                    class="input !p-2 5 text-sm font-medium inline-flex items-center gap-1.5 hover:text-blue-700">
            ...
            <!--[if BLOCK]><![endif]--><?php if(count($orderItems) > 1): ?>
                <button wire:click="removeOrderItem('<?php echo e($index); ?>')"
                        class="input !p-2 5 text-sm font-medium inline-flex items-center gap-1.5 hover:text-orange-600">
                    <svg class="size-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
