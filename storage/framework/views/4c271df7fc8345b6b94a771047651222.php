<div x-data="{state : $wire.entangle('state')}" x-show="state"
    ...
    <!--[if BLOCK]><![endif]--><?php if(!is_null($payment)): ?>
        <div class="grid gap-1">
            ...
            <input type="text" class="my-input" disabled value="<?php echo e($payment->pid ?? ""); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Qeyd</div>
            <textarea name="" class="my-input" rows="3" disabled><?php echo e($payment->note); ?></textarea>
        </div>
        <div class="grid gap-1">
            <div class="my-label">Tarix</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->created_at->format("d-m-Y h:i:s") ?? ""); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">İcraçı</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->executor->name ?? ""); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Müştəri</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->customer->name ?? ""); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Məbləğ</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->amount . " AZN" ?? ""); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Təsnifat</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->type->name); ?>">
        </div>
        <div class="grid gap-1">
            <div class="my-label">Əməliyyat</div>
            <input type="text" class="my-input" disabled value="<?php echo e($payment->action->name); ?>">
        </div>
        ...
        <!--[if BLOCK]><![endif]--><?php if($payment->is_cancelled): ?>
            <div class="grid gap-1">
                ...
                <input type="text" class="my-input" disabled value="<?php echo e($payment->cancelledUser->name); ?>">
            </div>
            <div class="grid gap-1">
                <div class="my-label">Ləğv tarixi</div>
                <input type="text" class="my-input" disabled value="<?php echo e($payment->updated_at->format("d-m-Y h:i:s")); ?>">
            </div>
            <div class="grid gap-1">
                <div class="my-label">Səbəb</div>
                <textarea name="" class="my-input" rows="3" disabled><?php echo e($payment->explanation); ?></textarea>
            </div>
        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->

        <a href="<?php echo e(url("payment/print/$payment->id")); ?>" class="inline-flex gap-1 items-center justify-center !p-3 font-semibold my-container">
